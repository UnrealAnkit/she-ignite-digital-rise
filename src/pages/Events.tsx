...
import { fetchUpcomingEvents, fetchPastEvents, Event } from "@/lib/eventService";
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {pastEvents.map((event, index) => {
                ...
                return (
                  <div key={event.id} className={`group relative overflow-hidden rounded-3xl shadow-lg hover:shadow-xl transition-all duration-300 ${cardVariant}`}>
                    {/* Decorative elements */}
                    <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16"></div>
                    <div className="absolute bottom-0 left-0 w-24 h-24 bg-white/10 rounded-full -ml-12 -mb-12"></div>
